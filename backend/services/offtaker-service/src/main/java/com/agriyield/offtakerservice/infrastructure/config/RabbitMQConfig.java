...
public class RabbitMQConfig {
    ...
    public static final String NDVI_UPDATED_QUEUE      = "offtaker.ndvi-updated.queue";
    ...
    private static final String NDVI_UPDATED_KEY      = "ndvi.updated";

    @Bean public TopicExchange offtakerExchange() {
        return new TopicExchange(OFFTAKER_EXCHANGE, true, false);
    }

    @Bean public TopicExchange geospatialExchange() {
        return new TopicExchange(GEOSPATIAL_EXCHANGE, true, false);
    }

    @Bean public Queue harvestPredictedQueue() {
        return QueueBuilder.durable(HARVEST_PREDICTED_QUEUE).build();
    }

    @Bean public Queue yieldPredictedQueue() {
        return QueueBuilder.durable(YIELD_PREDICTED_QUEUE).build();
    }

    @Bean public Queue ndviUpdatedQueue() {
        return QueueBuilder.durable(NDVI_UPDATED_QUEUE).build();
    }
