...
function DispatchStatusIcon({ status }: { status: string }) {
  const cls = "h-4 w-4 text-gray-600";
  switch (status) {
    case 'SCHEDULED':       return <Calendar className={cls} />;
    case 'ARRIVED':         return <Truck className={cls} />;
    case 'LOADED':          return <Package className={cls} />;
    case 'DELIVERED':       return <CheckCircle2 className="h-4 w-4 text-green-600" />;
    case 'DRIVER_DEFAULTED':return <AlertTriangle className="h-4 w-4 text-red-500" />;
...
export default function LogisticsPage() {
  ...
  const load = async () => {
    try {
      ...
      if (res.data.success) {
        const signedBids = (res.data.data || []).filter((b: Bid) =>
        ...
        for (const bid of signedBids) {
          ...
          try {
            ...
            if (dr.data.success) {
              setDispatches(prev => ({
                ...prev,
                [bid.agreementId as string]: dr.data.data || []
              }));
            }
          } catch {
            // No dispatches yet — fine
          }
        }
      }
    } catch {
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">
              Schedule and track harvest collection dispatches
            </p>
          </div>
          <button
        ...
        {showForm && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                >
                  ...
                  {bids
                    .filter(b => b.status === 'CONTRACT_SIGNED' && b.agreementId)
                    .map(b => (
                      <option key={b.agreementId!} value={b.agreementId!}>
                        {b.quantityQuintals} qt —{' '}
                        {b.totalValueEtb.toLocaleString()} ETB (
                        {b.agreementId!.slice(0, 8)}…)
                      </option>
                    ))}
                </select>
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={driverFaydaId}
                  onChange={e => setDriverFaydaId(e.target.value)}
                  required
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 mb-4 text-xs text-amber-700">
              A 500 ETB driver penalty deposit will be locked in escrow. This is
              forfeited if the driver defaults.
            </div>
        ...
        {bids.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">
              Dispatches are available after a purchase agreement is signed by both parties
            </p>
          </div>
        ) : (
                  ...
                  <div className="bg-gray-50 border-b border-gray-100 px-5 py-3 flex justify-between items-center">
                    <div>
                      <p className="font-bold text-gray-800 text-sm">
                        {bid.quantityQuintals} qt —{' '}
                        {bid.totalValueEtb.toLocaleString()} ETB
                      </p>
                          ...
                          >
                            ...
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              <div>
                                ...
                                <p className="font-semibold">
                                  {new Date(d.scheduledPickupDate).toLocaleDateString()}
                                </p>
                              </div>
                              {d.actualPickupDate && (
                                <div>
                                  ...
                                  <p className="font-semibold text-green-600">
                                    {new Date(d.actualPickupDate).toLocaleDateString()}
                                  </p>
                                </div>
                              )}
                              <div>
                                ...
                                <p className="font-semibold">
                                  {d.driverPenaltyEscrowEtb.toLocaleString()} ETB
                                </p>
                              </div>
                            </div>
                            ...
                            {d.status === 'LOADED' &&
                              confirmDispatch !== agreementId && (
                                <button
                                  onClick={() => setConfirmDispatch(agreementId)}
                            ...
                            {confirmDispatch === agreementId && (
                              <div className="mt-3 bg-green-50 border border-green-100 rounded-xl p-4 space-y-3">
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <button
                                    onClick={() => handleConfirmDelivery(agreementId)}
