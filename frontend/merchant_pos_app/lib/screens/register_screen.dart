...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> _register() async {
    ...
    final result = await _authService.register(
      ...
      businessLicenseNumber: _licenseController.text,
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Merchant Registration'), backgroundColor: Colors.orange, foregroundColor: Colors.white),
      body: Padding(
          ...
          child: Column(
            children: [
              if (!_otpSent) ...[
                ...
                TextField(controller: _licenseController, decoration: const InputDecoration(labelText: 'Business License Number', border: OutlineInputBorder())),
                const SizedBox(height: 12),
                TextField(controller: _nameController, decoration: const InputDecoration(labelText: 'Owner Full Name', border: OutlineInputBorder())),
                const SizedBox(height: 12),
                ...
                SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _register, style: ElevatedButton.styleFrom(backgroundColor: Colors.orange, padding: const EdgeInsets.symmetric(vertical: 16)), child: _loading ? const CircularProgressIndicator() : const Text('Register'))),
              ] else ...[
                ...
                const SizedBox(height: 24),
                SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _verifyOtp, style: ElevatedButton.styleFrom(backgroundColor: Colors.orange), child: _loading ? const CircularProgressIndicator() : const Text('Verify OTP'))),
              ],
            ],
          ),
        ),
