...
class _PriceTransparencyScreenState extends State<PriceTransparencyScreen> {
  ...
  Future<void> _load() async {
    ...
    final results = await Future.wait([
      ...
      _merchantService.getRedemptionHistory(),
    ]);
    if (mounted) {
      setState(() {
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      final anomaly = flagged
                          ? _anomalies.firstWhere((a) => a.productId == p.id)
                          : null;
                      ...
                      return Card(
                        ...
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(12),
                          side: BorderSide(
                            color: flagged ? Colors.orange : Colors.transparent,
                            width: flagged ? 1.5 : 0,
                          ),
                        ),
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  _priceBox('Your Price',
                                      'ETB ${p.currentPriceEtb.toStringAsFixed(2)}',
                                      flagged ? Colors.orange : Colors.green),
                                  const SizedBox(width: 12),
                                  if (anomaly != null)
                                    _priceBox(
                                      'Regional Median',
                              ...
                              if (flagged && anomaly != null) ...[
                                const SizedBox(height: 8),
                                Text(
                                  'Your price is ${anomaly.deviationPct.toStringAsFixed(1)}% above the regional median. Consider lowering to ETB ${anomaly.regionalMedianEtb.toStringAsFixed(2)}.',
                                  style: const TextStyle(
