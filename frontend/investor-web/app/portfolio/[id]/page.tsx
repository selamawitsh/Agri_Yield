      ...
      <div className="container mx-auto px-4 sm:px-6 py-6 max-w-4xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5">
            <div className="flex justify-between items-center mb-3">
              <div>
                <h3 className="font-bold text-gray-800 text-sm">Sentinel-2 Satellite</h3>
                <p className="text-xs text-gray-400">True-colour · Copernicus · Band 4/3/2</p>
              </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5">
            <div className="flex justify-between items-center mb-3">
              <div>
                <h3 className="font-bold text-gray-800 text-sm">Farm Location</h3>
                <p className="text-xs text-gray-400">
                  {farmMapData
                    ? `${farmMapData.areaHectares.toFixed(1)} ha · ${mapLat.toFixed(4)}°N, ${mapLng.toFixed(4)}°E`
                    : 'Approximate region coordinates'}
                </p>
              </div>
              <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full font-semibold">📍 Map</span>
            </div>
            <FarmMap
              lat={mapLat}
              lng={mapLng}
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 mb-6">
          ...
          <p className="text-xs text-gray-400 mb-4">
            Satellite vegetation index — last 90 days · {ndviChartData.length} reading{ndviChartData.length !== 1 ? 's' : ''}
          </p>
          {ndviChartData.length > 0
            ? <NdviChart data={ndviChartData} height={200} />
            : <div className="h-32 flex items-center justify-center bg-gray-50 rounded-2xl text-gray-400 text-sm">No NDVI readings yet</div>
          }
        </div>
        ...
        {yieldPred && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 mb-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: 'Min',  value: `${yieldPred.predictedYieldMin.toFixed(1)} qt/ha`,  color: 'text-gray-700' },
                { label: 'Mean', value: `${yieldPred.predictedYieldMean.toFixed(1)} qt/ha`, color: 'text-green-700' },
                { label: 'Max',  value: `${yieldPred.predictedYieldMax.toFixed(1)} qt/ha`,  color: 'text-gray-700' },
              ].map(y => (
                <div key={y.label} className="bg-gray-50 rounded-2xl p-4 text-center border border-gray-100">
                  <p className="text-xs text-gray-400 font-medium mb-1">{y.label} Yield</p>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 mb-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: 'Principal',       value: `${investment.amountEtb.toLocaleString()} ETB`,              bg: 'bg-gray-50',    color: 'text-gray-800' },
              { label: 'Expected Return', value: `+${expectedReturn.toFixed(0)} ETB`,                         bg: 'bg-green-50',   color: 'text-green-700' },
              { label: 'Total Payout',    value: `${(investment.amountEtb + expectedReturn).toFixed(0)} ETB`, bg: 'bg-emerald-50', color: 'text-emerald-700' },
            ].map(c => (
