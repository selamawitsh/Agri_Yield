...
export default function FarmMap({ lat, lng, label, height = 300, ndvi, areaHectares }: Props) {
  ...
  useEffect(() => {
    ...
    // Destroy existing map if container already initialized
    if (mapRef.current) {
    ...
    import('leaflet').then(L => {
      if (!ref.current) return;
      // Extra safety: remove any existing Leaflet instance on the container
      ...
      if (label) {
        ...
        marker.bindPopup(`<div style="font-family:sans-serif;min-width:140px"><strong style="color:#065f46">${label}</strong>${ndviText}${areaText}</div>`, { maxWidth: 200 }).openPopup();
      }
    });
