...
class _CropCycleScreenState extends State<CropCycleScreen> {
  ...
  static const Map<String, Map<String, String>> _strings = {
    'am': {
      'title':          'የሰብል ወቅቶች',
      'newSeason':      'አዲስ ወቅት',
      'newSeasonTitle': 'አዲስ የሰብል ወቅት',
      'seasonName':     'የወቅቱ ስም (አማራጭ)',
      'seasonHint':     'ለምሳሌ፡ Kiremt_2027',
      'harvestDate':    'የሚጠበቀው የምርት ቀን *',
      'cancel':         'ሰርዝ',
      'create':         'ፍጠር',
      'createFirst':    'የመጀመሪያ ወቅት ፍጠር',
      'noSeasons':      'ምንም ወቅቶች የሉም',
      'successCreate':  'አዲስ ወቅት ተፈጠረ!',
      'retry':          'እንደገና ሞክር',
      'expectedHarvest':'የሚጠበቀው ምርት',
      'plantingDate':   'የዘር ቀን',
      'actualHarvest':  'ትክክለኛ ምርት',
    },
    'om': {
      'title':          'Yeroolee Midhaan',
      'newSeason':      'Yeroo Haaraa',
      'newSeasonTitle': 'Yeroo Midhaan Haaraa',
      'seasonName':     'Maqaa Yeroo (dirqama miti)',
      'seasonHint':     'Fkn: Kiremt_2027',
      'harvestDate':    'Guyyaa Sassaabuu Eegamu *',
      'cancel':         'Haquu',
      'create':         'Uumi',
      'createFirst':    'Yeroo Jalqabaa Uumi',
      'noSeasons':      'Yeroon hin jiru',
      'successCreate':  'Yeroon haaraan uumame!',
      'retry':          'Irra deebi\'i yaalii',
      'expectedHarvest':'Sassaabuu Eegamu',
      'plantingDate':   'Guyyaa Facaasuu',
      'actualHarvest':  'Sassaabuu Dhugaa',
    },
    'en': {
      'title':          'Crop Seasons',
      'newSeason':      'New Season',
      'newSeasonTitle': 'New Crop Season',
      'seasonName':     'Season Name (optional)',
      'seasonHint':     'e.g. Kiremt_2027',
      'harvestDate':    'Expected Harvest Date *',
      'cancel':         'Cancel',
      'create':         'Create',
      'createFirst':    'Create First Season',
      'noSeasons':      'No seasons yet',
      'successCreate':  'New season created!',
      'retry':          'Retry',
  ...
  Future<void> _loadCycles() async {
    ...
    if (mounted) setState(() {
      _isLoading = false;
      if (result['success'] == true) _cycles = result['cropCycles'] as List<CropCycleModel>;
      else _error = result['message'];
    });
  }

  Future<void> _showCreateDialog() async {
    DateTime selectedDate = DateTime.now().add(const Duration(days: 120));
    final seasonController = TextEditingController();
    await showDialog(
      context: context,
      builder: (ctx) => StatefulBuilder(builder: (ctx, setDialogState) => AlertDialog(
        title: Text(_t('newSeasonTitle')),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: seasonController,
              decoration: InputDecoration(labelText: _t('seasonName'), hintText: _t('seasonHint'), border: const OutlineInputBorder())),
          const SizedBox(height: 16),
          InkWell(
            onTap: () async {
            ...
            child: InputDecorator(
              decoration: InputDecoration(labelText: _t('harvestDate'), border: const OutlineInputBorder(), prefixIcon: const Icon(Icons.calendar_today)),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final result = await _farmService.createCropCycle(
                  farmId: widget.farmId, expectedHarvestDate: dateStr,
                  seasonName: seasonController.text.isNotEmpty ? seasonController.text : null);
              if (mounted) {
                if (result['success'] == true) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(_t('successCreate')), backgroundColor: Colors.green));
                  _loadCycles();
                } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(icon: const Icon(Icons.refresh), onPressed: _loadCycles)],
      ),
      floatingActionButton: FloatingActionButton.extended(
        onPressed: _showCreateDialog, backgroundColor: Colors.green,
        icon: const Icon(Icons.add, color: Colors.white),
        label: Text(_t('newSeason'), style: const TextStyle(color: Colors.white)),
      ),
      body: _isLoading
        ...
        Text(_error!),
        ElevatedButton(onPressed: _loadCycles, child: Text(_t('retry'))),
      ]))
          : _cycles.isEmpty
          ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.loop, size: 64, color: Colors.grey.shade400), const SizedBox(height: 16),
        Text(_t('noSeasons')), const SizedBox(height: 8),
        ElevatedButton.icon(onPressed: _showCreateDialog,
            icon: const Icon(Icons.add), label: Text(_t('createFirst')),
            style: ElevatedButton.styleFrom(backgroundColor: Colors.green, foregroundColor: Colors.white)),
      ]))
          : RefreshIndicator(onRefresh: _loadCycles, color: Colors.green,
          child: ListView.builder(
            padding: const EdgeInsets.all(16), itemCount: _cycles.length,
            itemBuilder: (context, index) {
              final cycle = _cycles[index];
              final statusColor = _cycleStatusColor(cycle.status);
              final statusIcon = _cycleStatusIcon(cycle.status);
              return Card(
                margin: const EdgeInsets.only(bottom: 12),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 2,
                child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    CircleAvatar(backgroundColor: statusColor.withOpacity(0.1),
                        child: Icon(statusIcon, color: statusColor, size: 20)),
                    const SizedBox(width: 12),
                    Expanded(child: Text(cycle.seasonName, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
                    Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                        decoration: BoxDecoration(color: statusColor.withOpacity(0.1),
                            borderRadius: BorderRadius.circular(20), border: Border.all(color: statusColor)),
                        child: Text(cycle.statusLabel, style: TextStyle(color: statusColor, fontSize: 12, fontWeight: FontWeight.bold))),
                  ]),
                  const SizedBox(height: 12),
                  const Divider(), const SizedBox(height: 8),
                  _buildCycleRow(Icons.calendar_today, _t('expectedHarvest'), cycle.expectedHarvestDate),
                  if (cycle.plantingDate != null) _buildCycleRow(Icons.grass, _t('plantingDate'), cycle.plantingDate!),
                  if (cycle.actualHarvestDate != null) _buildCycleRow(Icons.agriculture, _t('actualHarvest'), cycle.actualHarvestDate!),
                ])),
              );
            },
          )),
