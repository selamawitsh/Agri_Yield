...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _verifyAccount(String id) async {
    final code = await showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(context, 'ETB1'), child: const Text('Verify')),
        ],
      ),
    );
    if (code == null) return;
    ...
    await _loadData();
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Account verified!')));
  }

  Future<void> _setDefault(String id) async {
    await _bankService.setDefaultAccount(id);
    await _loadData();
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Default account set')));
  }
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          if (!_showAddForm)
                            IconButton(icon: const Icon(Icons.add, color: Colors.green), onPressed: () => setState(() => _showAddForm = true)),
                        ],
                      ),
                      if (_loading) const Center(child: Padding(padding: EdgeInsets.all(32), child: CircularProgressIndicator()))
                        ...
                        const Padding(padding: EdgeInsets.all(32), child: Center(child: Text('No bank accounts linked'))),
                      if (_showAddForm)
                        Container(
                          ...
                          child: Column(
                            children: [
                              DropdownButtonFormField(
                                ...
                                items: const [DropdownMenuItem(value: 'TELEBIRR', child: Text('Telebirr')), DropdownMenuItem(value: 'CBE', child: Text('CBE Bank'))],
                                onChanged: (v) => setState(() => _selectedAccountType = v!),
                                decoration: const InputDecoration(labelText: 'Account Type'),
                              ),
                              TextField(controller: _accountNumberController, decoration: const InputDecoration(labelText: 'Account Number'), keyboardType: TextInputType.phone),
                              TextField(controller: _accountHolderController, decoration: const InputDecoration(labelText: 'Holder Name (Optional)')),
                              const SizedBox(height: 12),
                              Row(children: [Expanded(child: ElevatedButton(onPressed: _addAccount, child: const Text('Add'))), const SizedBox(width: 12), Expanded(child: OutlinedButton(onPressed: () => setState(() => _showAddForm = false), child: const Text('Cancel')))]),
                            ],
                          ),
                        ),
                      ..._accounts.map((acc) => Container(
                        ...
                        decoration: BoxDecoration(
                          border: Border.all(color: acc.isDefault ? Colors.green : Colors.grey),
                          borderRadius: BorderRadius.circular(8),
                          color: acc.isDefault ? Colors.green.shade50 : null,
                        ),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Row(
                              children: [
                                ...
                                if (acc.isDefault) const Chip(label: Text('Default'), backgroundColor: Colors.green, labelStyle: TextStyle(color: Colors.white), materialTapTargetSize: MaterialTapTargetSize.shrinkWrap),
                                if (acc.isVerified) const Chip(label: Text('Verified'), backgroundColor: Colors.green, labelStyle: TextStyle(color: Colors.white), materialTapTargetSize: MaterialTapTargetSize.shrinkWrap),
                                const Spacer(),
                                PopupMenuButton(
                                  onSelected: (v) {
